...
export const createMedical = createAsyncThunk(
  ...
  async (values) => {
    try {
      const response = await api.createMedical(values)
...
export const getRelatedTours = createAsyncThunk(
  ...
  const medicalSlice = createSlice({
    ...
    extraReducers: {
      [createMedical.pending]: (state, action) => {
        state.loading = true;
      },
      [createMedical.fulfilled]: (state, action) => {
        state.loading = false;
        state.projects = action.payload;
      },
      [createMedical.rejected]: (state, action) => {
        state.loading = false;
