...
const Boardingbgt = () => {
  ...
  useEffect(()=>{
    ...
    try {
      const res= await axios.get(`http://localhost:5000/boadingfinanace`)
...
{books.map((book)=>{
        ...
        return(
          <tbody>
           ...
           <td>*</td>  
              ...
              <td>{book.termlyBgt}</td>
              <td>{book.maintance}</td>
              <td>{book.remarks}</td>
    ...
    {/* </>:'hello'}  */}
      ...
      {expense.map((i)=>{
        return( 
          <tbody>
        <td>Total Boarding Expenses:{i.total+i.total2} </td>
      </tbody>  
   )
